#!/usr/bin/env python
# coding: utf-8

import json
import pandas as pd
from pathlib import Path

# Optional: remove unused imports if desired
# from pprint import pprint
# import matplotlib.pyplot as plt
# import networkx as nx
# import seaborn as sns


# ======================================================
# Load ATT&CK JSON (using paths relative to this file)
# ======================================================

BASE_DIR = Path(__file__).resolve().parent

enterprise_file = BASE_DIR / "enterprise-attack.json"
mobile_file = BASE_DIR / "mobile-attack.json"
ics_file = BASE_DIR / "ics-attack.json"

with open(enterprise_file, "r", encoding="utf-8") as f:
    stix = json.load(f)

objects = stix.get("objects", [])

# ======================================================
# Master dataframe
# ======================================================

master_df = pd.DataFrame([
    {
        "id": o.get("id"),
        "type": o.get("type"),
        "name": o.get("name"),
        "description": o.get("description"),
        "created": o.get("created"),
        "modified": o.get("modified"),
        "raw": o
    }
    for o in objects
])


# ======================================================
# Techniques (attack-patterns)
# ======================================================

tech_objs = [o for o in objects if o.get("type") == "attack-pattern"]

def technique_row(o):
    ext_refs = o.get("external_references", [])
    mitre_ref = next((r for r in ext_refs if r.get("source_name") == "mitre-attack"), {})
    external_id = mitre_ref.get("external_id")

    kcp = o.get("kill_chain_phases") or []
    phases = [
        p.get("phase_name")
        for p in kcp
        if isinstance(p, dict) and p.get("phase_name")
    ]

    platforms = (
        o.get("x_mitre_platforms")
        or o.get("x-mitre-platforms")
        or []
    )

    return {
        "id": o.get("id"),
        "tech_name": o.get("name"),
        "external_id": external_id,
        "description": o.get("description"),
        "platforms": platforms,
        "kill_chain_phases": phases,
        "raw": o
    }

tech_df = pd.DataFrame([technique_row(o) for o in tech_objs])


# ======================================================
# Tactics
# ======================================================

tactics = sorted({
    phase
    for phases in tech_df["kill_chain_phases"]
    for phase in phases
    if phase
})

tactics_df = pd.DataFrame({"tactic": tactics})


# ======================================================
# Relationships
# ======================================================

rel_objs = [o for o in objects if o.get("type") == "relationship"]

rel_df = pd.DataFrame([
    {
        "id": o.get("id"),
        "relationship_type": o.get("relationship_type"),
        "source_ref": o.get("source_ref"),
        "target_ref": o.get("target_ref"),
        "description": o.get("description"),
        "raw": o
    }
    for o in rel_objs
])


# ======================================================
# Groups (intrusion-sets)
# ======================================================

group_objs = [o for o in objects if o.get("type") == "intrusion-set"]

def group_row(o):
    return {
        "id": o.get("id"),
        "name": o.get("name"),
        "aliases": o.get("aliases", []),
        "description": o.get("description"),
        "created": o.get("created"),
        "modified": o.get("modified"),
        "raw": o
    }

groups_df = pd.DataFrame([group_row(g) for g in group_objs])


# ======================================================
# Group → Technique relationships
# ======================================================

uses_rels = [
    r for r in rel_objs
    if r.get("relationship_type") == "uses"
    and r.get("source_ref", "").startswith("intrusion-set--")
    and r.get("target_ref", "").startswith("attack-pattern--")
]

rows = []
tech_map = {t["id"]: t for t in tech_df.to_dict("records")}
group_map = {g["id"]: g for g in group_objs}

for r in uses_rels:
    group_id = r["source_ref"]
    tech_id = r["target_ref"]

    group_entry = group_map.get(group_id, {})
    tech_entry = tech_map.get(tech_id, {})

    rows.append({
        "group_id": group_id,
        "group_name": group_entry.get("name"),
        "technique_id": tech_id,
        "technique_name": tech_entry.get("tech_name"),
        "tactic": tech_entry.get("kill_chain_phases"),
        "technique_description": tech_entry.get("description"),
        "relationship_description": r.get("description"),
    })

group_techniques_df = pd.DataFrame(rows)


# ======================================================
# Technique counts per group
# ======================================================

tech_counts = (
    group_techniques_df
        .groupby("group_name")["technique_id"]
        .nunique()
        .reset_index(name="technique_count")
        .sort_values("technique_count", ascending=False)
)


# ======================================================
# Public exports (what gets imported from the package)
# ======================================================

__all__ = [
    "master_df",
    "tech_df",
    "tactics_df",
    "rel_df",
    "groups_df",
    "group_techniques_df",
    "tech_counts",
]
